...
def clean_string_basic(text):
    # split() 默认会按所有的空白字符分割，并自动去除多余的空格（包括首尾空格）
    # 然后用 ' '.join() 将分割后的单词用单个空格重新连接起来
...
def main(ref_utt, hyp, out_dir):
    ...
    for line_idx, line in tqdm.tqdm(enumerate(ref_lines), mininterval=0.01):
        ...
        for permu_idx, permu in enumerate(permutations(spkr_list)):
            ref = ' '.join(permu)
            hyp = clean_string_basic(hyp_lines[line_idx].strip().split('\t')[0].replace('$', ' '))
        ...
        best_hyps.append(clean_string_basic(hyp_lines[line_idx].strip().replace('$', ' ')))
