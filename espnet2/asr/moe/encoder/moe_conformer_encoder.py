...
from espnet2.asr.moe.nets.moe_positionwise_feed_forward import MOE_PositionwiseFeedForward
from espnet2.asr.moe.nets.moe_attention import (
    MOE_LegacyRelPositionMultiHeadedAttention,
    MOE_MultiHeadedAttention,
    MOE_RelPositionMultiHeadedAttention,
)
from espnet2.asr.moe.nets.moe_encoder_layer import MOE_EncoderLayer
...
class MOE_ConformerEncoder(AbsEncoder):
    ...
    def __init__(
        ...
        use_linear_moe: bool = False,
        use_att_moe: bool = False,
        num_experts: int = 1,
        lora_rank: int = 0,
        lora_alpha: int = 1,
        ...
        if positionwise_layer_type == "linear":
            if use_moe and use_linear_moe:
                positionwise_layer = MOE_PositionwiseFeedForward
        ...
        if selfattention_layer_type == "selfattn":
            ...
            if use_moe and use_att_moe:
                encoder_selfattn_layer = MOE_MultiHeadedAttention
            ...
            assert pos_enc_layer_type == "legacy_rel_pos"
            if use_moe and use_att_moe:
                encoder_selfattn_layer = MOE_LegacyRelPositionMultiHeadedAttention
                encoder_selfattn_layer_args = (
                    attention_heads,
                    output_size,
                    attention_dropout_rate,
            ...
            if use_moe and use_att_moe:
                encoder_selfattn_layer = MOE_RelPositionMultiHeadedAttention
                encoder_selfattn_layer_args = (
                    attention_heads,
                    output_size,
                    attention_dropout_rate,
    ...
    def forward(
        ...
        if self.global_router_fc:
            ...
            global_mask = masks
            global_weights, class_loss = self.global_router_fc(xs_pad_x, global_mask, audiomasks)
        ...
        return [xs_pad, class_loss], olens, None
        # return xs_pad, olens, None, class_loss
